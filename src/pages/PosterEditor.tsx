...
const PosterEditor = () => {
  ...
  const [selectedCategory, setSelectedCategory] = useState<'shapes' | 'graphics' | 'stickers' | 'photos' | 'videos' | 'audio' | 'forms' | 'sheets' | 'frames' | 'grids' | 'charts' | 'tables'>('stickers');

  const watermarkSrc = `${import.meta.env.BASE_URL}logo.png`;
  const stickerAssets = [
    { key: 'subscribe', label: 'Subscribe', src: `${import.meta.env.BASE_URL}stickers/subscribe.svg` },
    { key: 'like-share', label: 'Like & Share', src: `${import.meta.env.BASE_URL}stickers/like-share.svg` },
    { key: 'click-arrow', label: 'Click Arrow', src: `${import.meta.env.BASE_URL}stickers/click-arrow.svg` },
  ];
  const graphicAssets = [
    { key: 'star', label: 'Star', src: `${import.meta.env.BASE_URL}graphics/star.svg` },
    { key: 'burst', label: 'Burst', src: `${import.meta.env.BASE_URL}graphics/burst.svg` },
  ];
  const frameAssets = [
    { key: 'rounded', label: 'Rounded Frame', src: `${import.meta.env.BASE_URL}frames/rounded.svg` },
  ];
  const gridAssets = [
    { key: 'grid3x3', label: '3×3 Grid', src: `${import.meta.env.BASE_URL}grids/grid-3x3.svg` },
  ];
  const chartAssets = [
    { key: 'bar', label: 'Bar Chart', src: `${import.meta.env.BASE_URL}charts/bar.svg` },
  ];
  const tableAssets = [
    { key: 'table', label: 'Table', src: `${import.meta.env.BASE_URL}tables/table.svg` },
  ];
  const videoAssets = [
    { key: 'play', label: 'Play Card', src: `${import.meta.env.BASE_URL}videos/play-card.svg` },
  ];
  const audioAssets = [
    { key: 'music', label: 'Music Card', src: `${import.meta.env.BASE_URL}audio/music-card.svg` },
  ];
  const formAssets = [
    { key: 'form', label: 'Form Card', src: `${import.meta.env.BASE_URL}forms/form-card.svg` },
  ];
  const sheetAssets = [
    { key: 'sheet', label: 'Sheet Card', src: `${import.meta.env.BASE_URL}sheets/sheet-card.svg` },
  ];
  ...
  return (
    ...
    <div className="min-h-screen bg-background text-foreground p-4">
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        {/* Canva-style 3-pane layout: Left tools, Center canvas, Right inspector */}
        <div className="grid grid-cols-1 md:grid-cols-[280px_1fr_320px] gap-6">
          {/* Left sidebar: uploads, elements, global styles */}
          <aside className="md:h-[calc(100vh-160px)] overflow-y-auto space-y-4">
            ...
            <div className="rounded-lg border p-3 space-y-2">
              ...
              <div className="flex flex-wrap items-center gap-2">
                ...
                <Button variant="outline" onClick={()=>addCurvedTextElement(heading || 'Curved Text')}>Curved Text</Button>
              </div>

              {/* Category selector */}
              <div className="pt-2">
                <div className="text-xs text-muted-foreground">Browse categories</div>
                <div className="grid grid-cols-3 gap-2 mt-2">
                  {[
                    ...
                    { id: 'grids', label: 'Grids', icon: '#️⃣' },
                    { id: 'tables', label: 'Tables', icon: '📋' },
                  ].map((c)=> (
                    <button
                      key={c.id}
                      className={`flex flex-col items-center justify-center rounded-md border p-2 text-xs hover:bg-muted ${selectedCategory===c.id ? 'border-primary' : 'border-muted'}`}
              ...
              {selectedCategory === 'shapes' && (
                <div className="pt-2 space-y-2">
                  ...
                  <div className="flex flex-wrap items-center gap-2">
                    <Button variant="outline" onClick={addRectElement}>Rectangle</Button>
                    <Button variant="outline" onClick={addRectElement}>Rounded</Button>
                    <Button variant="outline" onClick={addRectElement}>Pill</Button>
                    <Button variant="outline" onClick={addRectElement}>Circle</Button>
                  </div>
                </div>
              )}
              ...
              {selectedCategory === 'graphics' && (
                <div className="pt-2 space-y-2">
                  <div className="text-xs text-muted-foreground">Graphics</div>
                  <div className="grid grid-cols-3 gap-2">
                    {graphicAssets.map(g => (
                      <button key={g.key} className="border rounded-md overflow-hidden hover:ring-2 ring-primary focus:outline-none" title={g.label} onClick={()=>addImageElement(g.src)}>
                        <img src={g.src} alt={g.label} className="w-full h-16 object-contain bg-muted" />
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {selectedCategory === 'frames' && (
                <div className="pt-2 space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {frameAssets.map(f => (
                      <button key={f.key} className="border rounded-md overflow-hidden hover:ring-2 ring-primary focus:outline-none" title={f.label} onClick={()=>addImageElement(f.src)}>
                        <img src={f.src} alt={f.label} className="w-full h-16 object-contain bg-muted" />
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {selectedCategory === 'charts' && (
                <div className="pt-2 space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {chartAssets.map(ch => (
                      <button key={ch.key} className="border rounded-md overflow-hidden hover:ring-2 ring-primary focus:outline-none" title={ch.label} onClick={()=>addImageElement(ch.src)}>
                        <img src={ch.src} alt={ch.label} className="w-full h-16 object-contain bg-muted" />
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {selectedCategory === 'tables' && (
                <div className="pt-2 space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {tableAssets.map(t => (
                      <button key={t.key} className="border rounded-md overflow-hidden hover:ring-2 ring-primary focus:outline-none" title={t.label} onClick={()=>addImageElement(t.src)}>
                        <img src={t.src} alt={t.label} className="w-full h-16 object-contain bg-muted" />
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {selectedCategory === 'stickers' && (
                <div className="pt-2 space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {stickerAssets.map((s)=> (
                      <button key={s.key} className="border rounded-md overflow-hidden hover:ring-2 ring-primary focus:outline-none" title={s.label} onClick={()=>addImageElement(s.src)}>
                        <img src={s.src} alt={s.label} className="w-full h-16 object-contain bg-muted" />
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {selectedCategory === 'photos' && (
                <div className="pt-2 space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {[`${import.meta.env.BASE_URL}test_image.png`, `${import.meta.env.BASE_URL}logo.png`].map((p, i)=> (
                      <button key={p + i} className="border rounded-md overflow-hidden hover:ring-2 ring-primary focus:outline-none" title="Photo" onClick={()=>addImageElement(p)}>
                        <img src={p} alt="Photo" className="w-full h-16 object-contain bg-muted" />
                      </button>
                    ))}
                  </div>
                </div>
              ...
              {selectedCategory === 'videos' && (
                <div className="pt-2 space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {videoAssets.map(v => (
                      <button key={v.key} className="border rounded-md overflow-hidden hover:ring-2 ring-primary focus:outline-none" title={v.label} onClick={()=>addImageElement(v.src)}>
                        <img src={v.src} alt={v.label} className="w-full h-16 object-contain bg-muted" />
                      </button>
                    ))}
                  </div>
                  <p className="text-xs text-muted-foreground">Note: These are design placeholders, not playable videos.</p>
                </div>
              )}
              ...
              {selectedCategory === 'audio' && (
                <div className="pt-2 space-y-2">
                  ...
                  <div className="grid grid-cols-3 gap-2">
                    {audioAssets.map(a => (
                      <button key={a.key} className="border rounded-md overflow-hidden hover:ring-2 ring-primary focus:outline-none" title={a.label} onClick={()=>addImageElement(a.src)}>
                        <img src={a.src} alt={a.label} className="w-full h-16 object-contain bg-muted" />
                      </button>
                    ))}
                  </div>
                  <p className="text-xs text-muted-foreground">Note: Design badges only, not audio players.</p>
                </div>
              )}
            ...
            {/* Typography & Colors (moved into sidebar) */}
            <div className="rounded-lg border p-3 space-y-3">
            ...
            {/* Quick Presets (moved into sidebar) */}
            <div className="rounded-lg border p-3 space-y-2">
          ...
          {/* Center: canvas editor with zoom/export */}
          <main className="space-y-3">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                <span className="text-xs text-muted-foreground">Zoom</span>
                <input type="range" min={0.5} max={2} step={0.05} value={stageScale} onChange={(e)=>setStageScale(Number(e.target.value))} />
                <span className="text-xs">{Math.round(stageScale*100)}%</span>
              </div>
              <Button onClick={exportPosterFromStage} disabled={!bgImageObj}>Export Poster</Button>
            </div>
                            ...
                            <KonvaText
                              ...
                              onDragMove={(e)=>{
                                ...
                                if (Math.abs(centerX - cx) < threshold) {
                                  vx = cx;
                                  (e.target as any).x((e.target as any).x() + (cx - centerX));
                                }
                                if (Math.abs(centerY - cy) < threshold) {
                                  hy = cy;
                                  (e.target as any).y((e.target as any).y() + (cy - centerY));
                                }
                                setGuides({ vx, hy });
                              }}
                              onDragEnd={(e)=>{ setGuides({}); updateSelected({ x: (e.target as any).x(), y: (e.target as any).y() })}}
                              onTransformEnd={(e)=>{
                    ...
                    {cropActive && cropRect ? (
                      <Layer>
                        <KonvaRect
                          ...
                          draggable
                          onDragEnd={(e)=>{
                            setCropRect({ x: (e.target as any).x(), y: (e.target as any).y(), width: cropRect.width, height: cropRect.height });
                          }}
                        />
        ...
        {/* Sidebar moved sections above; center and footer remain below */}
